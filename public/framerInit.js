...
(function() {
  ...
  if (!window.AlturaDivinaCart) {
    ...
    window.AlturaDivinaCart = {
      ...
      closeCart: function() {
        // Solo para compatibilidad API
        console.log('Cerrando carrito (modo Framer)');
      },
    ...
    window.mpCartCount = cartData.totalItems || 0;
      console.log('API de carrito fallback inicializada para Framer con sessionId:', sessionId);
    
    // Añadir función de verificación de conexión
    window.checkCartConnection = function() {
      return {
        initialized: true,
        sessionId: window.mpSessionId,
        cartCount: window.mpCartCount,
        hasAlturaDivinaCart: !!window.AlturaDivinaCart,
